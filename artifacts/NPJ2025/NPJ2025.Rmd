...
include_graphics("figures/figure_2_.pdf")
...
plot_b = model_summaries %>% 
  ...
  mutate(condition = case_when(condition == "holdout" ~ "Generalization",
                               condition == "train" ~ "Train")) %>% 
  mutate(condition = factor(condition, levels = c("Train", "Generalization"))) %>% 
  ggplot(aes(value_z, fill = interaction(factor(condition), factor(hidden_units)))) +
  #geom_violin() +
  #geom_boxplot(width = .5, outlier.size = .2) +
  geom_density() +
  theme_apa() +
...
top<- plot_grid(plot_a, plot_c, ncol = 2, labels = c("A", "B"))

# Arrange the bottom row plot centered
bottom <- plot_grid(NULL, plot_b, NULL, ncol = 3, rel_widths = c(.5, 1.5, .5), labels = c("C"))
