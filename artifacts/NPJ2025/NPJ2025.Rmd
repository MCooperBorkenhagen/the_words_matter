...
train_sd = model_summaries %>% 
  filter(metric == "mse" & condition == "train") %>%
  summarise(SD = first(SD)) %>% 
  pull(SD)

plot_b = model_summaries %>% 
  filter(metric == "mse") %>% 
  #group_by(condition) %>% 
  mutate(#value_z_within_train = (value - M)/train_sd,
         condition = case_when(condition == "train" ~ "Train",
                               condition == "holdout" ~ "Generalization")) %>% 
  ggplot(aes(value_z, fill = condition)) +
  geom_density() +
  theme_apa() +
...
Data and code in order to reproduce all analyses included here can be found at the project's page on the Open Science Framework (XX).
...
In addition to the subsampling procedure and following the steps in Brauer & Curtin (2018), we also used the "bound optimization by quadratic approximation" optimizer implemented in the $lmer()$ function in the lme4 package in R (Bates et al., 2015). The procedure for the sampling procedure and for all other analyses can be found in the project's OSF repository (XX). Note that in the code, "ensemble" is indicated with the variable "model_id" and "representational capacity" is associated with the variable name "hidden_units".
