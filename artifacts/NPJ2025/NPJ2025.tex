...
Words were collected from popular children's books, taken from the corpus developed in Lewis et al.~(2021). All monosyllabic words (\(N\) = 2,869) from the Lewis et al.~(2021) corpus comprised the pool of words from which training and generalization items were sampled (no multisyllabic words were included). Learning was weighted by frequency such that during each learning trial the error signal associated with any word presented to the model was proportional to its frequency as established in the Hypertext Analog of Language corpus (Lund \& Burgess, 1996; accessed through data from English Lexicon Project, see Balota et al., 2007)\footnote{See appendix for a technical description of how the scaling is accomplished.}. Table 1 presents descriptive statistics for both the full dataset and the subset of the data that were only used to evaluate generalization (in a holdout set never used for model training). The values for the descriptive statistics did not show a significant difference between the holdout items and the others: letters, \(F\)(1, 2868) = 3.38, \(p\) = .07; phonemes, \(F\)(1, 2774) = 3.25, \(p\) = .07; frequency \(F\)(1, 2773) = 1.08, \(p\) = .30\footnote{Note that degrees of freedom are different due to missingness attributed to the database used for the measure.}.
...
\begin{tabular}{llll}
\toprule
Set & \multicolumn{1}{c}{Mean (SD)} & \multicolumn{1}{c}{Minimum (Z)} & \multicolumn{1}{c}{Maximum (Z)}\\
\midrule
Train & .004 (.001) & .002 (-3.04) & .006 (3.90)\\
Generalization & .013 (.001) & .01 (-3.48) & .02 (4.41)\\
\bottomrule
\addlinespace
\end{tabular}
...
\begin{figure}
\centering
\includegraphics{figures/figure_2_.pdf}
...
\midrule
20 & 0.004 (1.06) & 0.0004 (.68) & 0.003 (-1.28) & 0.006 (3.90)\\
30 & 0.003 (-.35) & 0.0003 (.63) & 0.002 (-2.65) & 0.005 (2.58)\\
40 & 0.003 (-.71) & 0.0003 (.63) & 0.002 (-3.04) & 0.005 (2.12)\\
\bottomrule
\addlinespace
\end{tabular}
...
Representational Capacity & \multicolumn{1}{c}{Mean (Z)} & \multicolumn{1}{c}{SD (Z)} & \multicolumn{1}{c}{Min (Z)} & \multicolumn{1}{c}{Max (Z)}\\
\midrule
20 & 0.013 (.84) & 0.0006 (0.79) & 0.011 (-1.93) & 0.016 (4.41)\\
30 & 0.012 (-.30) & 0.0006 (0.80) & 0.010 (-3.16) & 0.015 (3.11)\\
40 & 0.012 (-.54) & 0.0006 (0.80) & 0.010 (-3.48) & 0.015 (2.81)\\
\bottomrule
\addlinespace
\end{tabular}
...
\begin{figure}
\centering
\includegraphics{NPJ2025_files/figure-latex/figure3-1.pdf}
...
In addition to examining generally whether or not performance differs in training and generalization words across levels of representational capacity, the experiment is able to answer a more specific question: would a learner perform comparably (relative to all other learners) if you could change their representational capacity to map orthography to phonology? We examined this by computing the rank correlation between the lowest (20) and highest (40) levels of hidden units in mean squared error in the holdout words, resulting in \(\rho\) = 0.89 (\(p\) \textless{} .001, 95\% CI = {[}0.89, 0.90{]}).
