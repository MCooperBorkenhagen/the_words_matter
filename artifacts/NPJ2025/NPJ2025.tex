...
\urlstyle{same}
\hypersetup{
  pdftitle={The Words Matter in Early Reading Acquisition: A Simulation Study},
...
\title{The Words Matter in Early Reading Acquisition: A Simulation Study}
...
Correspondence concerning this article should be addressed to Matthew J. Cooper Borkenhagen, 1114 W. Call Street, Tallahassee, FL 32306. E-mail: \href{mailto:mcooperborkenhagen@fsu.com}{\nolinkurl{mcooperborkenhagen@fsu.com}}
...
\normalsize{\textit{Note.} 300 holdout items were selected from 2,869 items in the full dataset.}
\end{tablenotes}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{img/figure_1.jpg}}
...
\subsection{Representational Capacity}\label{representational-capacity}
...
\emph{Note.} The difference between the pronunciation for ``breathed'' and ``bruised'' is four features (also sometimes referred to as ``units''). Pronunciations are transcribed using the one-letter version of the ARPAbet. The change in number of features (Diff. features) is relative to the top row (``breathed''). Spellings for nonwords are hypothetical given the pronunciation. The estimates of change in standardized MSE (Diff. MSE Standardized) are taken from standardized data for training and generalization. Accuracy is provided for reference, though results in the paper are explained in terms of mean squared error. Note that the differences across successive rows are defined by differences in individual features, though they also result in differences in phonemes. It is not always the case that a difference of one feature results in a legal phoneme of English.

~

\subsection{Additional Details}\label{additional-details}

The simulation and analysis code can all be found in the project's Github repository (github.com/XXX). Additional information can be found in the appendix and data for the project can be found in OSF (\url{https://osf.io/qvu8m/?view_only=a17142c524924219ad3951ac85324d8e}).
...
Models in general achieved low error on the training words (\emph{M} = .0029, \emph{SD} = .00048) relative to generalization (\emph{M} = .012, \emph{SD} = .00076), and the distribution for training and generalization words was non-overlapping. Noteworthy variation existed within and across these two sets, data for which is provided in Table 3.
...
\midrule
Train & .0029 (-11.8) & 0.00048 (.62) & 0.0013 (-13.9) & .005 (-9.04)\\
Generalization & .012 (0) & 0.00076 (1) & .0093 (-3.39) & .015 (4.03)\\
\bottomrule
\addlinespace
\end{tabular}

\begin{tablenotes}[para]
\normalsize{\textit{Note.} Error is provided in raw form and standardized form. The standardized form appears in parentheses, where standardization happens relative to the generalization set as in other analyses here. These values are calculated across all words in each set (train and generalization), collapsing across representational capacity conditions.}
\end{tablenotes}
...
\midrule
20 & 0.013 (0.88) & 0.00059 (0.78) & 0.0103 (-2.03) & 0.0149 (4.03)\\
30 & 0.012 (-0.29) & 0.00058 (0.77) & 0.0097 (-2.88) & 0.0142 (3.06)\\
40 & 0.011 (-0.59) & 0.00059 (0.77) & 0.0093 (-3.39) & 0.0138 (2.51)\\
\bottomrule
\addlinespace
\end{tabular}
...
Representational capacity & \multicolumn{1}{c}{Mean (Z)} & \multicolumn{1}{c}{SD (Z)} & \multicolumn{1}{c}{Min (Z)} & \multicolumn{1}{c}{Max (Z)}\\
\midrule
20 & 0.0034 (-11.11) & 0.00031 (0.41) & 0.0022 (-12.75) & 0.005 (-9.04)\\
30 & 0.0027 (-12.01) & 0.00029 (0.38) & 0.0015 (-13.61) & 0.0041 (-10.15)\\
40 & 0.0025 (-12.25) & 0.00028 (0.37) & 0.0013 (-13.85) & 0.0041 (-10.24)\\
\bottomrule
\addlinespace
\end{tabular}

\begin{tablenotes}[para]
\normalsize{\textit{Note.} For each value the standardized (Z) version of MSE is shown in parentheses.}
\end{tablenotes}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{NPJ2025_files/figure-latex/figure3-1.pdf}}
\caption{\label{fig:figure3}The distribution of training and generalization items for each model at the end of training for standardized mean squared error. Performance on the training words was in general much better (lower error) than on the generalization words (A). Noteworthy variation exists within each set, leading to dramatic differences in performance for the lowest and highest achieving models. Data are displayed in terms of standardized mean squared error to afford an interpretation in standard deviation units, where standardization takes place within the generalization set.}
\end{figure}

The effect of word, ensemble, and representational capacity bears out in a linear mixed effects model predicting mean squared error (standardized) from number of hidden units and using model comparisons with specifications for different random effects \footnote{See supplement for additional information about model specification, including details regarding the steps taken to support model convergence for the linear mixed effects models.}. A model with random intercepts for word and ensemble had a better fit than a model without an ensemble random intercept, \(\chi^2\)(1) = 3069.80, \(p\) \textless{} .001. In addition, a model with a fixed effect for hidden units provided a better fit than one without such an effect, \(\chi^2\)(1) = 5015.90, \(p\) \textless{} .001.

An increase of one hidden unit is associated with a reduction of .01 units of standardized mean squared error (\(b\) = -0.01, \(SE\) = 0.0001, \(p\) \textless{} 0.001). Given that hidden unit was manipulated in increments of 10 in our simulations, this estimate translated to a .10 standard deviation decrease in MSE for every 10 additional hidden units. Table 6 displays the results of this final model. Note however that these model estimated pertain to the subset obtained and used for statistical modeling, which was 10\% of all the data from the experiments performed \footnote{See supplement for information about sampling procedure, model estimation, and the rationale related to how convergence problems were managed in the linear mixed effects models estimated.}.
...
\midrule
Intercept & 0.20 & 0.04 & 300 & 5.00 & < 0.001 \\
Hidden & -0.01 & 0.0001 & 300 & -70.92 & < 0.001 \\
\midrule
\textbf{Variance Components} & \textbf{SD} & & & & \\
\midrule
\multicolumn{6}{l}{\textit{Ensemble}} \\
\hspace{1em}Intercept & 0.053 & & & & \\
\midrule
\multicolumn{6}{l}{\textit{Word}} \\
\hspace{1em}Intercept & 0.47 & & & & \\
\textit{Residual} & 0.52 & & & & \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Differences in Outcomes of Specific Ensembles at Different Levels of Representational Capacity}\label{differences-in-outcomes-of-specific-ensembles-at-different-levels-of-representational-capacity}

In addition to examining generally whether or not performance differs in training and generalization words across levels of representational capacity, the experiment is able to answer a more specific question: would a learner perform comparably (relative to all other learners) if you could change their representational capacity to map orthography to phonology? We examined this by computing the rank correlation between the lowest (20) and highest (40) levels of hidden units in mean squared error in the holdout words, resulting in \(\rho\) = 0.88 (\(p\) \textless{} .001, 95\% CI = {[}0.87, 0.88{]}).
...
Apfelbaum, K. S., Hazeltine, E., \& McMurray, B. (2013). Statistical learning in reading: Variability in irrelevant letters helps children learn phonics skills. Developmental Psychology, 49(7), 1348–1365. https://doi.org/10.1037/a0030817
...
Cooper Borkenhagen, M. J., \& Cox, C. R. (2025, May). Connectionist models of word reading. PsyArXiv. https://XXX
...
Cunningham, A. E., \& Stanovich, K. E. (1993). Children's literacy environments and early word recognition subskills. Reading and Writing, 5, 193-204. https://doi.org/10.1007/BF01027484
...
Seidenberg, M. S., Waters, G. S., Barnes, M. A., \& Tanenhaus, M. K. (1984). When does irregular spelling or pronunciation influence word recognition? Journal of Verbal Learning and Verbal Behavior, 23(3), 383–404. https://doi.org/10.1016/S0022-5371(84)90270-2

\end{hangparas}

\section{Data Availability Statement}\label{data-availability-statement}

Data and code in order to reproduce all analyses included here can be found at the project's page on the Open Science Framework (\url{https://osf.io/qvu8m/?view_only=a17142c524924219ad3951ac85324d8e}).

\section{Code Availability Statement}\label{code-availability-statement}

Code to run the simulations reported here can be found at the project's GitHub page (XX).

\section{Appendix}\label{appendix}

\subsection{Frequency Transformation}\label{frequency-transformation}

The frequencies were transformed in the same way as other connectionist approaches using similar networks (Seidenberg \& McClelland, 1989) using the formula: \(p = K \cdot \log(\text{frequency} + 1)\)). A log transformation was applied to all the frequencies, which was in turn multiplied by a value, \(K\). This resulted in values that were scaled between 0 and 1, which was captured the idea that for very frequent words the error signal should be high (i.e., approximately 1 times the loss for that word) and very infrequent words should receive relatively weak signal resulting from the corresponding calculation of loss for that word. In order to account for the fact that some words were not present in the Lund \& Burgess (1996) corpus a constant was added to each frequency prior to being converted to log form in order to push very low values slightly away from zero. This resulted in a distribution such that the most frequent word ``the'' had a scaled value (p) of 1 and the least frequent values had values of near zero (\(1 \times 10^{-6}\)).

\subsection{Notes on the Estimation of Linear Mixed Effects Models}\label{notes-on-the-estimation-of-linear-mixed-effects-models}

Because of the large scale of the dataset, a random subset of the data was obtained for statistical model after initial attempts to arrive at a converged model failed with the entire dataset. In order to perform this subset, we randomly selected 1000 of the 10000 model ID values and saved item-level performance data for training and holdout words for that 1000 ensemble subset. Also, the complexity of the models estimated resulted in non-convergence when the maximal random effects structure was used (see Barr, Levy, Scheepers, \& Tily, 2013 for a discussion of the benefits and challenges with using a maximal random effects structure). Numerous steps were taken to support convergence, following the advice of Brauer \& Curtin (2018). Ultimately, we removed the random slopes for hidden unit, which models to converge. As suggested by Brauer \& Curtin (2018) and by Barr et al., (2013), we advise the reader that in some circumstances removing random slopes in linear mixed effects models can result in an inflated false positive rate. However, we will note that this advice (which originated with Bare et al., 2013) is based on simulation data with far fewer observations per item (circumstances of 12 or 25 items respectively) than our own (see Barr et al., 2013 for more discussion).

In addition to the subsampling procedure and following the steps in Brauer \& Curtin (2018), we also used the ``bound optimization by quadratic approximation'' optimizer implemented in the \(lmer()\) function in the lme4 package in R (Bates et al., 2015). The procedure for the sampling procedure and for all other analyses can be found in the project's OSF repository (\url{https://osf.io/qvu8m/?view_only=a17142c524924219ad3951ac85324d8e}). Note that in the code, ``ensemble'' is indicated with the variable ``model\_id'' and ``representational capacity'' is associated with the variable name ``hidden\_units''.
